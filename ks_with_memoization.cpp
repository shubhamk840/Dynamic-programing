...
int ks(vector<pair<int,int>> &prs,int k,int n)
{
	...
	{
		if(prs[n-1].first<=k) // when weight of the item is less than the k
		{
			return dp[k][n]=max(prs[n-1].second+ks(prs,k-prs[n-1].first,n-1),ks(prs,k,n-1));
			// considered both the choices.
		}
		else  //  when weight is grater, 
		{
			return dp[k][n]=ks(prs,k,n-1);
	    }
	}
}
int main()

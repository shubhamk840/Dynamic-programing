...
int solve(string first,string second,int x,int y)
{
	if(x==0||y==0)
	return 0;
	else
	{
		...
		{
			return 1+solve(first,second,x-1,y-1);
		}
		else
		{
			return max(solve(first,second,x-1,y),solve(first,second,x,y-1));
		}
	}
}
...
int main()
{
    ...
    cin>>first;
    cin>>second;
    cout<<solve(first,second,first.length(),second.length())<<endl;
}
